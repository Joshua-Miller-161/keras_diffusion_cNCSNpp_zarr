...
def get_act(config):
    logger.info(f" >> >> INSIDE get_act {config.model.nonlinearity} {type(config.model.nonlinearity)}")
    """Get activation functions from the config file."""
    if str(config.model.nonlinearity).lower() == 'elu':
        return nn.ELU()
    elif str(config.model.nonlinearity).lower() == 'relu':
        return nn.ReLU()
    elif str(config.model.nonlinearity).lower() == 'lrelu':
        return nn.LeakyReLU(negative_slope=0.2)
    elif str(config.model.nonlinearity).lower() in ['swish', 'silu']:
        return nn.SiLU()
    else:
        raise NotImplementedError('activation function does not exist!')
